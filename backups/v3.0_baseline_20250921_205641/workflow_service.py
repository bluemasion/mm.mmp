...
class MaterialWorkflowService:
    ...
    def _initialize_data(self):
        ...
        try:
            ...
            self.df_master = load_master_data()
            
            if not self.df_master.empty:
                logging.info(f"已成功加载主数据，共 {len(self.df_master)} 条记录")
            else:
                logging.warning("主数据为空")
                
        except Exception as e:
            logging.error(f"主数据加载失败: {e}")
            # 最后的回退选项
            try:
                self.df_master = load_csv_data(config.MASTER_DATA_PATH)
                logging.info("已从默认文件加载主数据")
            except:
                self.df_master = pd.DataFrame()
                logging.error("所有数据加载方法均失败")
